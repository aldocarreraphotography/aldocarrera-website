...
/* ════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════ */
function MarkupCanvas({ imgUrl, markups, tool, color, onAdd, readOnly }) {
  ...
  // Redraw when markups change / image loads / on resize
  useEffect(() => { redraw(); }, [markups, imgUrl, redraw]);
  useEffect(() => {
...
/* ════════════════════════════════════════════════════════
   REVIEW LIGHTBOX
   ════════════════════════════════════════════════════════ */
function ReviewLightbox({ images, idx, setIdx, onClose, onFeedback, onMarkupAdd, onMarkupDelete }) {
  ...
  if (!img) return null;
  const fb = img.feedback || { label: null, stars: 0, note: '', markups: [] };
...
function UnlockScreen({ token, onUnlocked }) {
  ...
  // Try an "open" unlock immediately (no creds) — if the gallery is open, we're in.
  useEffect(() => {
    (async () => {
      try {
        const r = await ugFetch('POST', `/api/ug/${token}/unlock`, '', {});
        if (r?.ok && r.key === '') onUnlocked(r);
      } catch (_) { /* needs creds */ }
    })();
  }, [token]);
  ...
  const submit = async (pin) => {
    ...
    try {
      const r = await ugFetch('POST', `/api/ug/${token}/unlock`, '', { pin });
      onUnlocked(r);
    } catch (e) {
  ...
  return (
    <>
      ...
      {lbIdx !== null && !isDelivery && (
        <ReviewLightbox images={images} idx={lbIdx} setIdx={setLbIdx}
